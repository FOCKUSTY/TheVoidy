...
export class SubcommandsDeployer {
  ...
  public execute(name: string) {
    return Object.fromEntries(
      readdirSync(join(this.dirname))
        .filter((file) => REG_EXP.test(file))
        .map((file) => {
          const subcommand = require(join(this.dirname, file))
            .default as typeof Subcommand<Response>;
          this.logger.execute(`Субкоманда ${name.toLowerCase()}>${subcommand.name}`);
          return subcommand;
        })
        .map((subcommand) => [subcommand.name.toLowerCase(), subcommand])
    ) as { [key: string]: typeof Subcommand<Response> };
  }
}
