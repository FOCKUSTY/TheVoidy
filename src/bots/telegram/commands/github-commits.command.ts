...
import { Commit, DEFAULT_PRESETS, REPO_OWNERS, RepoOwners, Services } from "@types";
import GitHubApi from "@bot/utility/github/github.utility";
  ...
  public constructor(services: Services<{ github: GitHubApi }>) {
    super({
      ...
      async execute(interaction) {
        ...
        const ownerType = command.ownerType as RepoOwners;

        if (!REPO_OWNERS.includes(ownerType))
        ...
        try {
          const data = JSON.parse(
            command.data || JSON.stringify(DEFAULT_PRESETS),
            undefined
          );
          ...
          const text = services.telegram.Format({
            ...
            pattern: {
              ...
              visualisation:
                data.visualisation || DEFAULT_PRESETS.visualisation
            },
          ...
          try {
            const text = services.telegram.Format({
              ...
              pattern: {
                repos: objectRepos,
                visualisation: DEFAULT_PRESETS.visualisation
              },
              linkEnabled: linksEnabled
            });
