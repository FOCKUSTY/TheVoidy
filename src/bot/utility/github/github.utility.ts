...
import { Branch, BranchCommit, GitHubApi as GitHubApiClass, Repo } from "@types";
...
class GitHubApi extends GitHubApiClass {
  public async getRepositories(owner: string, type: string, ignoredRepo: string[] = [".github"]) {
    ...
    try {
      ...
      const repositories: Repo[] | { status: number } = await data.json();
